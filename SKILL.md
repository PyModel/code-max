...
Choose by risk, not size. Escalate when the work turns out riskier than it looked.

| Mode | When | Loop |
| --- | --- | --- |
| Focused | Local behavior change with a clear contract | inspect → change → focused regression → diff review |
| Substantial | Several components, public behavior, or an unclear contract | Focused + acceptance checklist + broader/integration checks |
| High-risk | Any trigger below | Substantial + migration, rollback, observability, independent review |
...
Inspect → contract + invariant + risk → baseline → smallest coherent slice → verify
  ...
  pass    → review diff → rerun affected checks → reconcile acceptance → report
```
...
deployment order, generated artifacts. A missing tool is not evidence of a code defect.

**Contract and invariant.** Identify the violated invariant before choosing the patch.
When sources disagree, investigate in this order: explicit user requirement and observed
runtime behavior → public contracts/schemas → tests → implementation → docs/comments →
inference. For Substantial work and above, keep a short acceptance checklist in the task
context; write it into the repository only if the repo already has a tracking convention
or the user asks.
...
**Verify.** Run the strongest relevant checks the project supports. Checks must exercise
the acceptance criteria and be able to fail. A skipped, unavailable, or failing check is not
a pass. Never delete, skip, or weaken legitimate tests or thresholds to get green. When a
check contradicts the apparent contract, investigate before changing either side; if
evidence cannot establish intended behavior, keep both and report the ambiguity.

**Review.** Read the diff as a domain engineer: boundaries, failure paths, security,
concurrency, compatibility, resources. Remove debug artifacts, dead code, and unrelated
formatting. High-risk work gets an independent read-only review when available, scoped to
correctness; say so when none ran. Self-review, including of a child agent's work, is not
independent. Rerun affected checks after the last relevant edit; an old green run does not
verify a new tree. Load [quality gates](references/quality-gates.md) for affected domains.
...
Fix in-scope defects; never hide out-of-scope ones.
...
| Status | Meaning |
| --- | --- |
| `COMPLETE` | All authorized acceptance criteria are satisfied by the strongest practical evidence for the task's risk level; no known material in-scope defect or unresolved assumption remains. |
